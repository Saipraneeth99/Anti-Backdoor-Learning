...
def train(opt):
    ...
    print('----------- Data Initialization of SIG--------------')
    data_path_isolation = os.path.join(opt.isolate_data_root, "{}_isolation{}%_SIGExamples.npy".format(opt.model_name,
                                                                                                    opt.isolation_ratio * 100))
    data_path_other = os.path.join(opt.isolate_data_root, "{}_other{}%_SIGExamples.npy".format(opt.model_name,
...
def save_checkpoint(state, epoch, is_best, opt):
    if is_best:
        filepath = os.path.join(opt.unlearning_root, opt.model_name + r'-SIGUnlearning_epochs{}.tar'.format(epoch))
