...
def isolate_data(opt, poisoned_data, losses_idx):
    ...
    # Save data
    if opt.save:
        data_path_isolation = os.path.join(opt.isolate_data_root, "{}_isolation{}%_NormalSIGExamples.npy".format(opt.model_name,
                                                                                             opt.isolation_ratio * 100))
        data_path_other = os.path.join(opt.isolate_data_root, "{}_other{}%_NormalSIGExamples.npy".format(opt.model_name,
...
def train_step(opt, train_loader, model_ascent, optimizer, criterion, epoch):
    ...
    for idx, (img, target) in enumerate(train_loader, start=1):
        ...
        elif opt.gradient_ascent_type == 'Normal':
            output = model_ascent(img)
...
def save_checkpoint(state, epoch, is_best, opt):
    if is_best:
        filepath = os.path.join(opt.isolate_model_root, opt.model_name + r'normal-tuning_epochs{}.tar'.format(epoch))
